...
SET IMAGE_NAME=caption-this-app-frontend-simple
SET BASE_DIR=%cd%

REM Build the image based on the Dockerfile
winpty docker build -t %IMAGE_NAME% -f Dockerfile .
...
winpty docker run  --rm --name %IMAGE_NAME% -ti --mount type=bind,source="%cd%",target=/app -p 8080:8080 %IMAGE_NAME%
