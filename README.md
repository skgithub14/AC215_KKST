...
This is the repository for the final project of Harvard [AC215 Fall 2021](https://harvard-iacs.github.io/2021-AC215/) Course - Advanced Practical Data Science (DevOps for Deep Learning). 
...
## App Design and Key Components
![Schematic of App Design](src/app_design.png)

#### API Server
We built an API server to serve these models using FastAPI. Upon startup, the API server downloads the saved model weights from a GCP bucket, and when receiving an input image, it returns the generated caption for that image.<br>

The [**api-service**](https://github.com/skgithub14/AC215_KKST/tree/main/api-service) folder contains codes for the models and API server

#### App Frontend
We used React.js to build the frontend of the Image Captioning App. The frontend allows users to upload an image and select a model type for captioning, and makes requests to the API. It then displays the caption generated by the selected model.<br>

The [**frontend-react**](https://github.com/skgithub14/AC215_KKST/tree/main/frontend-react) folder contains codes for the React frontend 

#### Deployment

The App is then deployed onto Kubernetes Clusters on Google Cloud Platform. We stored the Docker images for both the API server and React frontend to Google Container Registry, and then created virtual machines that run these containers on the cluster with Kubernetes.<br>
